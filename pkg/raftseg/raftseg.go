...
import (
    ...
    "github.com/marciobarbosa/url-shortener/pkg/raftdisk"
)
...
func _LoadSegment(seg *segmenthdl) (error) {
    ...
    for {
	ksizeblob, nbytes := raftdisk.ReadData(seg.file, offset, intsize)
	if nbytes <= 0 {
	    return nil
	}
	...
	kblob, nbytes := raftdisk.ReadData(seg.file, offset, keysize)
	offset += uint64(nbytes)

	vsizeblob, nbytes := raftdisk.ReadData(seg.file, offset, intsize)
	offset += uint64(nbytes)
...
func _RecoverFromCrash() (error) {
    ...
    for _, rec := range dir {
	file, err := raftdisk.OpenFile(recovery + "/" + rec.Name())
	if err != nil {
	    raftdisk.CloseAllFiles()
	    return err
	}
    ...
    if nfiles == 0 {
	recfile, err = raftdisk.OpenFile(recovery + "/" + "0")
	return err
    }
    if nfiles != 1 {
	raftdisk.CloseAllFiles()
    ...
    recfile, err = raftdisk.OpenFile(recovery + "/" + strconv.Itoa(index))
    if err != nil {
	raftdisk.CloseAllFiles()
	return err
    }
    ...
    for {
	ksizeblob, nbytes := raftdisk.ReadData(rfile, offset, intsize)
	if nbytes <= 0 {
	    raftdisk.RemoveFile(rfile)
	    return nil
	}
	offset += uint64(nbytes)

	keysize := binary.LittleEndian.Uint64(ksizeblob)
	kblob, nbytes := raftdisk.ReadData(rfile, offset, keysize)
	offset += uint64(nbytes)

	vsizeblob, nbytes := raftdisk.ReadData(rfile, offset, intsize)
	offset += uint64(nbytes)
	valsize := binary.LittleEndian.Uint64(vsizeblob)
...
func Init(params InitParams) (error) {
	...
	file, err := raftdisk.OpenFile(seg.Name())
	if err != nil {
	    raftdisk.CloseAllFiles()
	    return err
	}
...
func _FlushEntry(seg *segmenthdl, key []byte, value []byte) (error) {
    ...
    offset, err := raftdisk.AppendData(seg.file, data)
...
func _MergeSegments() (error) {
    ...
    newfile, _ := raftdisk.OpenFile(strconv.Itoa(segindex))
	...
	for seg_it := segments.Back().Prev(); seg_it != nil; seg_it = prev {
	    ...
	    data, nbytes := raftdisk.ReadData(seg.file, offset, valsize)
	    offset += uint64(nbytes)
...
func _CreateSegment() {
    ...
    file, _ := raftdisk.OpenFile(strconv.Itoa(segindex))
...
func _RecoverLogAdd(key []byte, value []byte) (error) {
    ...
    _, err := raftdisk.AppendData(recfile, data)
    return err
}
...
func GetDisk(key []byte) ([]byte, error) {
    ...
    data, nbytes := raftdisk.ReadData(seg.file, offset, vsize)
    if nbytes <= 0 {
...
func LoadDist() (error) {
    ...
    file, _ := raftdisk.OpenFile(segname)
    seghdl.index = segindex
    seghdl.file = file
